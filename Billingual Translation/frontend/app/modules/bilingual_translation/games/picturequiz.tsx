...
export default function PictureQuiz({ category, playerName = "Player" }: { category?: string; playerName?: string }) {
  ...
  const loadWords = useCallback(async () => {
    ...
    try {
      ...
      const unmasteredWords = sortedData.filter(
        word => !previouslyCorrectWords.has(word.tamil_word)
      );

      if (unmasteredWords.length === 0) {
        setQuestions([]);
        setError("You have mastered all the available image words! Try a different category or wait for new words.");
        setIsLoading(false);
        return;
      }
      ...
      const randomWords = shuffleArray(adaptiveData);

      // Prevent repeated images in the same game session.
      const uniqueImageWords = Array.from(
        new Map(randomWords.map((word) => [typeof word.image_url === 'number' ? word.image_url : String(word.image_url).toLowerCase(), word])).values()
      );

      const questionWords = uniqueImageWords.slice(0, maxQuestions);
